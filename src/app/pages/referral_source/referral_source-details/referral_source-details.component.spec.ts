...
import { CReferral_SourceComponent } from './c-referral_source.component';

describe('CReferral_SourceComponent', () => {
  let component: CReferral_SourceComponent;
  let fixture: ComponentFixture<CReferral_SourceComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchReferral_SourceErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getReferral_SourceById: value => ({ subscribe: f => f({}) }),
      updateReferral_Source: (value, value1) => ({ subscribe: f => f({}) }),
      addReferral_Source: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CReferral_SourceComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CReferral_SourceComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getReferral_SourceById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getReferral_SourceById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updateReferral_Source').and.callThrough();
      spyOn(appsServiceStub, 'addReferral_Source').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updateReferral_Source).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addReferral_Source).toHaveBeenCalled();
    });
  });
});
