...
@Component({
  selector: 'app-referral_source-details',
  templateUrl: './referral_source-details.component.html',
  styleUrls: ['./referral_source-details.component.css']
})
export class Referral_SourceDetailsComponent implements OnInit {
...
isActive: ['', [Validators.required]],
   ...
   this.dataService.getReferral_SourceById(this.id).subscribe((data: any)=> {
      this.referral_sourceDetails = data;
  ...
  submitForm(): void {
   ...
   this.isLoading = true;
    this.dataService.updateReferral_Source(this.id, tempFormData).subscribe(async(data: any) => {
...
fileChange(event: any, propertyName: any) {
  ...
  uploadImage(propertyName: any) {
    ...
    formData.append('tableName', 'Referral_Source');
