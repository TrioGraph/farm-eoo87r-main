...
import { VDocumentsComponent } from './v-Documents.component';

describe('VDocumentsComponent', () => {
  let component: VDocumentsComponent;
  let fixture: ComponentFixture<VDocumentsComponent>;
  ...
  beforeEach(() => {
    ...
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetch<<Documents>>ErrorMessage) => ({})
    });
    const appsServiceStub = () => ({
      validatePermissionWithRole: editTaskAcivity => ({}),
      get<<Documents>>: () => ({ pipe: () => ({ subscribe: f => f({}) }) }),
      deleteRecord: (string, guid) => ({ subscribe: f => f({}) })
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [VDocumentsComponent],
      providers: [
        ...
        { provide: FarmService, useFactory: documentsServiceStub }
      ]
    });
    fixture = TestBed.createComponent(VDocumentsComponent);
  ...
  describe('fetchPermissions', () => {
    it('makes expected calls', () => {
      const documentsServiceStub: DocumentsService = fixture.debugElement.injector.get(
        FarmService
      );
      spyOn(documentsServiceStub, 'validatePermissionWithRole').and.callThrough();
      component.fetchPermissions();
      expect(documentsServiceStub.validatePermissionWithRole).toHaveBeenCalled();
    });
  });
  ...
  describe('fetchData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(documentsServiceStub, 'getDocuments').and.callThrough();
      component.fetchData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(documentsServiceStub.getDocuments).toHaveBeenCalled();
    });
  });
});
