...
import { CCampaignsComponent } from './c-campaigns.component';

describe('CCampaignsComponent', () => {
  let component: CCampaignsComponent;
  let fixture: ComponentFixture<CCampaignsComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchCampaignsErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getCampaignsById: value => ({ subscribe: f => f({}) }),
      updateCampaigns: (value, value1) => ({ subscribe: f => f({}) }),
      addCampaigns: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CCampaignsComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CCampaignsComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getCampaignsById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getCampaignsById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updateCampaigns').and.callThrough();
      spyOn(appsServiceStub, 'addCampaigns').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updateCampaigns).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addCampaigns).toHaveBeenCalled();
    });
  });
});
