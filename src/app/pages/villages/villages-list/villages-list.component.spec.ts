...
import { VVillagesComponent } from './v-Villages.component';

describe('VVillagesComponent', () => {
  let component: VVillagesComponent;
  let fixture: ComponentFixture<VVillagesComponent>;
  ...
  beforeEach(() => {
    ...
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetch<<Villages>>ErrorMessage) => ({})
    });
    const appsServiceStub = () => ({
      validatePermissionWithRole: editTaskAcivity => ({}),
      get<<Villages>>: () => ({ pipe: () => ({ subscribe: f => f({}) }) }),
      deleteRecord: (string, guid) => ({ subscribe: f => f({}) })
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [VVillagesComponent],
      providers: [
        ...
        { provide: FarmService, useFactory: villagesServiceStub }
      ]
    });
    fixture = TestBed.createComponent(VVillagesComponent);
  ...
  describe('fetchPermissions', () => {
    it('makes expected calls', () => {
      const villagesServiceStub: VillagesService = fixture.debugElement.injector.get(
        FarmService
      );
      spyOn(villagesServiceStub, 'validatePermissionWithRole').and.callThrough();
      component.fetchPermissions();
      expect(villagesServiceStub.validatePermissionWithRole).toHaveBeenCalled();
    });
  });
  ...
  describe('fetchData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(villagesServiceStub, 'getVillages').and.callThrough();
      component.fetchData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(villagesServiceStub.getVillages).toHaveBeenCalled();
    });
  });
});
