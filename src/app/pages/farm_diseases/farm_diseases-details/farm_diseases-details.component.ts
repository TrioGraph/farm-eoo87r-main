...
@Component({
  selector: 'app-farm_diseases-details',
  templateUrl: './farm_diseases-details.component.html',
  styleUrls: ['./farm_diseases-details.component.css']
})
export class Farm_DiseasesDetailsComponent implements OnInit {
...
this.dataService.getPhotosLookup().subscribe((result: any) => { 
   ...
   this.dataService.getFarm_DiseasesById(this.id).subscribe((data: any)=> {
      this.farm_diseasesDetails = data;
  ...
  submitForm(): void {
   ...
   this.isLoading = true;
    this.dataService.updateFarm_Diseases(this.id, tempFormData).subscribe(async(data: any) => {
