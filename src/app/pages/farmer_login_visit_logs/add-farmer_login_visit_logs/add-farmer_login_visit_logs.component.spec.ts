...
import { CFarmer_login_visit_logsComponent } from './c-farmer_login_visit_logs.component';

describe('CFarmer_login_visit_logsComponent', () => {
  let component: CFarmer_login_visit_logsComponent;
  let fixture: ComponentFixture<CFarmer_login_visit_logsComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchFarmer_login_visit_logsErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getFarmer_login_visit_logsById: value => ({ subscribe: f => f({}) }),
      updateFarmer_login_visit_logs: (value, value1) => ({ subscribe: f => f({}) }),
      addFarmer_login_visit_logs: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CFarmer_login_visit_logsComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CFarmer_login_visit_logsComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getFarmer_login_visit_logsById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getFarmer_login_visit_logsById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updateFarmer_login_visit_logs').and.callThrough();
      spyOn(appsServiceStub, 'addFarmer_login_visit_logs').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updateFarmer_login_visit_logs).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addFarmer_login_visit_logs).toHaveBeenCalled();
    });
  });
});
