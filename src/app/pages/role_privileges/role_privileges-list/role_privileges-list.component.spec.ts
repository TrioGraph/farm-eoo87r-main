...
import { VRole_PrivilegesComponent } from './v-Role_Privileges.component';

describe('VRole_PrivilegesComponent', () => {
  let component: VRole_PrivilegesComponent;
  let fixture: ComponentFixture<VRole_PrivilegesComponent>;
  ...
  beforeEach(() => {
    ...
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetch<<Role_Privileges>>ErrorMessage) => ({})
    });
    const appsServiceStub = () => ({
      validatePermissionWithRole: editTaskAcivity => ({}),
      get<<Role_Privileges>>: () => ({ pipe: () => ({ subscribe: f => f({}) }) }),
      deleteRecord: (string, guid) => ({ subscribe: f => f({}) })
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [VRole_PrivilegesComponent],
      providers: [
        ...
        { provide: FarmService, useFactory: role_privilegesServiceStub }
      ]
    });
    fixture = TestBed.createComponent(VRole_PrivilegesComponent);
  ...
  describe('fetchPermissions', () => {
    it('makes expected calls', () => {
      const role_privilegesServiceStub: Role_PrivilegesService = fixture.debugElement.injector.get(
        FarmService
      );
      spyOn(role_privilegesServiceStub, 'validatePermissionWithRole').and.callThrough();
      component.fetchPermissions();
      expect(role_privilegesServiceStub.validatePermissionWithRole).toHaveBeenCalled();
    });
  });
  ...
  describe('fetchData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(role_privilegesServiceStub, 'getRole_Privileges').and.callThrough();
      component.fetchData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(role_privilegesServiceStub.getRole_Privileges).toHaveBeenCalled();
    });
  });
});
