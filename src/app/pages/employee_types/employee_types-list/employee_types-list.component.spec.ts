...
import { VEmployee_TypesComponent } from './v-Employee_Types.component';

describe('VEmployee_TypesComponent', () => {
  let component: VEmployee_TypesComponent;
  let fixture: ComponentFixture<VEmployee_TypesComponent>;
  ...
  beforeEach(() => {
    ...
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetch<<Employee_Types>>ErrorMessage) => ({})
    });
    const appsServiceStub = () => ({
      validatePermissionWithRole: editTaskAcivity => ({}),
      get<<Employee_Types>>: () => ({ pipe: () => ({ subscribe: f => f({}) }) }),
      deleteRecord: (string, guid) => ({ subscribe: f => f({}) })
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [VEmployee_TypesComponent],
      providers: [
        ...
        { provide: FarmService, useFactory: employee_typesServiceStub }
      ]
    });
    fixture = TestBed.createComponent(VEmployee_TypesComponent);
  ...
  describe('fetchPermissions', () => {
    it('makes expected calls', () => {
      const employee_typesServiceStub: Employee_TypesService = fixture.debugElement.injector.get(
        FarmService
      );
      spyOn(employee_typesServiceStub, 'validatePermissionWithRole').and.callThrough();
      component.fetchPermissions();
      expect(employee_typesServiceStub.validatePermissionWithRole).toHaveBeenCalled();
    });
  });
  ...
  describe('fetchData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(employee_typesServiceStub, 'getEmployee_Types').and.callThrough();
      component.fetchData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(employee_typesServiceStub.getEmployee_Types).toHaveBeenCalled();
    });
  });
});
