...
export class Employee_TypesListComponent  implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.allowToAdd = localStorage.getItem('AccessList')?.split(',').includes(Privileges.AddEmployeeTypes.toString()) ? true : false; 
    this.allowToEdit = localStorage.getItem('AccessList')?.split(',').includes(Privileges.UpdateEmployeeTypes.toString()) ? true : false;  
    this.allowToDelete = localStorage.getItem('AccessList')?.split(',').includes(Privileges.DeleteEmployeeTypes.toString()) ? true : false;  
  }

getData(startDate: any) {
    this.isLoading = true;
    if(startDate && startDate.Length > 0) {
        this.dataService.searchEmployee_Types('', 1, 10, '', '', true, 'm.Created_Date','string', '>', startDate, '').subscribe((result: any)=> {
...
(async(error: any) => {
      ...
      this.dataService.searchEmployee_Types('', 1, 10, '', '', false, '','', '', '', '').subscribe((result: any)=> {
        this.employee_typesList = result.data;
        this.isLoading = false;
	},
(async(error: any) => {
  ...
  searchEmployee_Types(searchText: any) {
    this.isLoading = true;
    this.dataService.searchEmployee_Types(searchText, 1, 10, '', '', false, '','', '', '', '').subscribe((result: any)=> {
      this.employee_typesList = result.data;
       this.isLoading = false;
     },
(async(error: any) => {
...
setRecordId(selectedId: any) {
  ...
  deleteRecord(ev: any) {
    ...
    {
      this.isLoading = true;
      this.dataService.updateEmployee_TypesStatus(this.selectedId).subscribe(async(result: any)=> {
      this.isLoading = false;
	const toast = await this.toastController.create({
