...
import { CBroadcast_MessageComponent } from './c-broadcast_message.component';

describe('CBroadcast_MessageComponent', () => {
  let component: CBroadcast_MessageComponent;
  let fixture: ComponentFixture<CBroadcast_MessageComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchBroadcast_MessageErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getBroadcast_MessageById: value => ({ subscribe: f => f({}) }),
      updateBroadcast_Message: (value, value1) => ({ subscribe: f => f({}) }),
      addBroadcast_Message: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CBroadcast_MessageComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CBroadcast_MessageComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getBroadcast_MessageById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getBroadcast_MessageById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updateBroadcast_Message').and.callThrough();
      spyOn(appsServiceStub, 'addBroadcast_Message').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updateBroadcast_Message).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addBroadcast_Message).toHaveBeenCalled();
    });
  });
});
