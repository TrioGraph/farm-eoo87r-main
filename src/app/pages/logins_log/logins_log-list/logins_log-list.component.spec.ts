...
import { VLogins_LogComponent } from './v-Logins_Log.component';

describe('VLogins_LogComponent', () => {
  let component: VLogins_LogComponent;
  let fixture: ComponentFixture<VLogins_LogComponent>;
  ...
  beforeEach(() => {
    ...
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetch<<Logins_Log>>ErrorMessage) => ({})
    });
    const appsServiceStub = () => ({
      validatePermissionWithRole: editTaskAcivity => ({}),
      get<<Logins_Log>>: () => ({ pipe: () => ({ subscribe: f => f({}) }) }),
      deleteRecord: (string, guid) => ({ subscribe: f => f({}) })
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [VLogins_LogComponent],
      providers: [
        ...
        { provide: FarmService, useFactory: logins_logServiceStub }
      ]
    });
    fixture = TestBed.createComponent(VLogins_LogComponent);
  ...
  describe('fetchPermissions', () => {
    it('makes expected calls', () => {
      const logins_logServiceStub: Logins_LogService = fixture.debugElement.injector.get(
        FarmService
      );
      spyOn(logins_logServiceStub, 'validatePermissionWithRole').and.callThrough();
      component.fetchPermissions();
      expect(logins_logServiceStub.validatePermissionWithRole).toHaveBeenCalled();
    });
  });
  ...
  describe('fetchData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(logins_logServiceStub, 'getLogins_Log').and.callThrough();
      component.fetchData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(logins_logServiceStub.getLogins_Log).toHaveBeenCalled();
    });
  });
});
