...
import { CRolesComponent } from './c-roles.component';

describe('CRolesComponent', () => {
  let component: CRolesComponent;
  let fixture: ComponentFixture<CRolesComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchRolesErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getRolesById: value => ({ subscribe: f => f({}) }),
      updateRoles: (value, value1) => ({ subscribe: f => f({}) }),
      addRoles: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CRolesComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CRolesComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getRolesById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getRolesById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updateRoles').and.callThrough();
      spyOn(appsServiceStub, 'addRoles').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updateRoles).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addRoles).toHaveBeenCalled();
    });
  });
});
