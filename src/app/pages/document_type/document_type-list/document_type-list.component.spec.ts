...
import { VDocument_TypeComponent } from './v-Document_Type.component';

describe('VDocument_TypeComponent', () => {
  let component: VDocument_TypeComponent;
  let fixture: ComponentFixture<VDocument_TypeComponent>;
  ...
  beforeEach(() => {
    ...
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetch<<Document_Type>>ErrorMessage) => ({})
    });
    const appsServiceStub = () => ({
      validatePermissionWithRole: editTaskAcivity => ({}),
      get<<Document_Type>>: () => ({ pipe: () => ({ subscribe: f => f({}) }) }),
      deleteRecord: (string, guid) => ({ subscribe: f => f({}) })
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [VDocument_TypeComponent],
      providers: [
        ...
        { provide: FarmService, useFactory: document_typeServiceStub }
      ]
    });
    fixture = TestBed.createComponent(VDocument_TypeComponent);
  ...
  describe('fetchPermissions', () => {
    it('makes expected calls', () => {
      const document_typeServiceStub: Document_TypeService = fixture.debugElement.injector.get(
        FarmService
      );
      spyOn(document_typeServiceStub, 'validatePermissionWithRole').and.callThrough();
      component.fetchPermissions();
      expect(document_typeServiceStub.validatePermissionWithRole).toHaveBeenCalled();
    });
  });
  ...
  describe('fetchData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(document_typeServiceStub, 'getDocument_Type').and.callThrough();
      component.fetchData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(document_typeServiceStub.getDocument_Type).toHaveBeenCalled();
    });
  });
});
