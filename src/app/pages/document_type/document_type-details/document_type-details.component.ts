...
@Component({
  selector: 'app-document_type-details',
  templateUrl: './document_type-details.component.html',
  styleUrls: ['./document_type-details.component.css']
})
export class Document_TypeDetailsComponent implements OnInit {
...
isActive: ['', [Validators.required]],
   ...
   this.dataService.getDocument_TypeById(this.id).subscribe((data: any)=> {
      this.document_typeDetails = data;
  ...
  submitForm(): void {
   ...
   this.isLoading = true;
    this.dataService.updateDocument_Type(this.id, tempFormData).subscribe(async(data: any) => {
...
fileChange(event: any, propertyName: any) {
  ...
  uploadImage(propertyName: any) {
    ...
    formData.append('tableName', 'Document_Type');
