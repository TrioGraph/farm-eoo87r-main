...
import { CTraining_VideosComponent } from './c-training_videos.component';

describe('CTraining_VideosComponent', () => {
  let component: CTraining_VideosComponent;
  let fixture: ComponentFixture<CTraining_VideosComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchTraining_VideosErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getTraining_VideosById: value => ({ subscribe: f => f({}) }),
      updateTraining_Videos: (value, value1) => ({ subscribe: f => f({}) }),
      addTraining_Videos: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CTraining_VideosComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CTraining_VideosComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getTraining_VideosById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getTraining_VideosById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updateTraining_Videos').and.callThrough();
      spyOn(appsServiceStub, 'addTraining_Videos').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updateTraining_Videos).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addTraining_Videos).toHaveBeenCalled();
    });
  });
});
