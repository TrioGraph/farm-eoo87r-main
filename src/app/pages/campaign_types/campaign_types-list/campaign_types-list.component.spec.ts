...
import { VCampaign_TypesComponent } from './v-Campaign_Types.component';

describe('VCampaign_TypesComponent', () => {
  let component: VCampaign_TypesComponent;
  let fixture: ComponentFixture<VCampaign_TypesComponent>;
  ...
  beforeEach(() => {
    ...
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetch<<Campaign_Types>>ErrorMessage) => ({})
    });
    const appsServiceStub = () => ({
      validatePermissionWithRole: editTaskAcivity => ({}),
      get<<Campaign_Types>>: () => ({ pipe: () => ({ subscribe: f => f({}) }) }),
      deleteRecord: (string, guid) => ({ subscribe: f => f({}) })
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [VCampaign_TypesComponent],
      providers: [
        ...
        { provide: FarmService, useFactory: campaign_typesServiceStub }
      ]
    });
    fixture = TestBed.createComponent(VCampaign_TypesComponent);
  ...
  describe('fetchPermissions', () => {
    it('makes expected calls', () => {
      const campaign_typesServiceStub: Campaign_TypesService = fixture.debugElement.injector.get(
        FarmService
      );
      spyOn(campaign_typesServiceStub, 'validatePermissionWithRole').and.callThrough();
      component.fetchPermissions();
      expect(campaign_typesServiceStub.validatePermissionWithRole).toHaveBeenCalled();
    });
  });
  ...
  describe('fetchData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(campaign_typesServiceStub, 'getCampaign_Types').and.callThrough();
      component.fetchData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(campaign_typesServiceStub.getCampaign_Types).toHaveBeenCalled();
    });
  });
});
