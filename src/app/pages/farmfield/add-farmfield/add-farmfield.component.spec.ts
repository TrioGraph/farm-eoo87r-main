...
import { CFarmFieldComponent } from './c-farmfield.component';

describe('CFarmFieldComponent', () => {
  let component: CFarmFieldComponent;
  let fixture: ComponentFixture<CFarmFieldComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchFarmFieldErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getFarmFieldById: value => ({ subscribe: f => f({}) }),
      updateFarmField: (value, value1) => ({ subscribe: f => f({}) }),
      addFarmField: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CFarmFieldComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CFarmFieldComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getFarmFieldById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getFarmFieldById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updateFarmField').and.callThrough();
      spyOn(appsServiceStub, 'addFarmField').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updateFarmField).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addFarmField).toHaveBeenCalled();
    });
  });
});
