...
import { VFarmFieldComponent } from './v-FarmField.component';

describe('VFarmFieldComponent', () => {
  let component: VFarmFieldComponent;
  let fixture: ComponentFixture<VFarmFieldComponent>;
  ...
  beforeEach(() => {
    ...
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetch<<FarmField>>ErrorMessage) => ({})
    });
    const appsServiceStub = () => ({
      validatePermissionWithRole: editTaskAcivity => ({}),
      get<<FarmField>>: () => ({ pipe: () => ({ subscribe: f => f({}) }) }),
      deleteRecord: (string, guid) => ({ subscribe: f => f({}) })
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [VFarmFieldComponent],
      providers: [
        ...
        { provide: FarmService, useFactory: farmfieldServiceStub }
      ]
    });
    fixture = TestBed.createComponent(VFarmFieldComponent);
  ...
  describe('fetchPermissions', () => {
    it('makes expected calls', () => {
      const farmfieldServiceStub: FarmFieldService = fixture.debugElement.injector.get(
        FarmService
      );
      spyOn(farmfieldServiceStub, 'validatePermissionWithRole').and.callThrough();
      component.fetchPermissions();
      expect(farmfieldServiceStub.validatePermissionWithRole).toHaveBeenCalled();
    });
  });
  ...
  describe('fetchData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(farmfieldServiceStub, 'getFarmField').and.callThrough();
      component.fetchData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(farmfieldServiceStub.getFarmField).toHaveBeenCalled();
    });
  });
});
