...
import { CNursaryComponent } from './c-nursary.component';

describe('CNursaryComponent', () => {
  let component: CNursaryComponent;
  let fixture: ComponentFixture<CNursaryComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchNursaryErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getNursaryById: value => ({ subscribe: f => f({}) }),
      updateNursary: (value, value1) => ({ subscribe: f => f({}) }),
      addNursary: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CNursaryComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CNursaryComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getNursaryById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getNursaryById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updateNursary').and.callThrough();
      spyOn(appsServiceStub, 'addNursary').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updateNursary).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addNursary).toHaveBeenCalled();
    });
  });
});
