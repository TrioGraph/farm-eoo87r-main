...
import { CNursary_ActivityComponent } from './c-nursary_activity.component';

describe('CNursary_ActivityComponent', () => {
  let component: CNursary_ActivityComponent;
  let fixture: ComponentFixture<CNursary_ActivityComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchNursary_ActivityErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getNursary_ActivityById: value => ({ subscribe: f => f({}) }),
      updateNursary_Activity: (value, value1) => ({ subscribe: f => f({}) }),
      addNursary_Activity: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CNursary_ActivityComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CNursary_ActivityComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getNursary_ActivityById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getNursary_ActivityById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updateNursary_Activity').and.callThrough();
      spyOn(appsServiceStub, 'addNursary_Activity').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updateNursary_Activity).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addNursary_Activity).toHaveBeenCalled();
    });
  });
});
