...
import { CFarmersComponent } from './c-farmers.component';

describe('CFarmersComponent', () => {
  let component: CFarmersComponent;
  let fixture: ComponentFixture<CFarmersComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchFarmersErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getFarmersById: value => ({ subscribe: f => f({}) }),
      updateFarmers: (value, value1) => ({ subscribe: f => f({}) }),
      addFarmers: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CFarmersComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CFarmersComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getFarmersById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getFarmersById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updateFarmers').and.callThrough();
      spyOn(appsServiceStub, 'addFarmers').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updateFarmers).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addFarmers).toHaveBeenCalled();
    });
  });
});
