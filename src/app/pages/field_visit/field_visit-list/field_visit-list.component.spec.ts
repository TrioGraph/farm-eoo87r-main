...
import { VField_VisitComponent } from './v-Field_Visit.component';

describe('VField_VisitComponent', () => {
  let component: VField_VisitComponent;
  let fixture: ComponentFixture<VField_VisitComponent>;
  ...
  beforeEach(() => {
    ...
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetch<<Field_Visit>>ErrorMessage) => ({})
    });
    const appsServiceStub = () => ({
      validatePermissionWithRole: editTaskAcivity => ({}),
      get<<Field_Visit>>: () => ({ pipe: () => ({ subscribe: f => f({}) }) }),
      deleteRecord: (string, guid) => ({ subscribe: f => f({}) })
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [VField_VisitComponent],
      providers: [
        ...
        { provide: FarmService, useFactory: field_visitServiceStub }
      ]
    });
    fixture = TestBed.createComponent(VField_VisitComponent);
  ...
  describe('fetchPermissions', () => {
    it('makes expected calls', () => {
      const field_visitServiceStub: Field_VisitService = fixture.debugElement.injector.get(
        FarmService
      );
      spyOn(field_visitServiceStub, 'validatePermissionWithRole').and.callThrough();
      component.fetchPermissions();
      expect(field_visitServiceStub.validatePermissionWithRole).toHaveBeenCalled();
    });
  });
  ...
  describe('fetchData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(field_visitServiceStub, 'getField_Visit').and.callThrough();
      component.fetchData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(field_visitServiceStub.getField_Visit).toHaveBeenCalled();
    });
  });
});
