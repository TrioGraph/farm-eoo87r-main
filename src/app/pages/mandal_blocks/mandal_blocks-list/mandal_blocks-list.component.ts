...
export class Mandal_BlocksListComponent  implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.allowToAdd = localStorage.getItem('AccessList')?.split(',').includes(Privileges.AddMandalBlocks.toString()) ? true : false; 
    this.allowToEdit = localStorage.getItem('AccessList')?.split(',').includes(Privileges.UpdateMandalBlocks.toString()) ? true : false;  
    this.allowToDelete = localStorage.getItem('AccessList')?.split(',').includes(Privileges.DeleteMandalBlocks.toString()) ? true : false;  
  }

getData(startDate: any) {
    this.isLoading = true;
    if(startDate && startDate.Length > 0) {
        this.dataService.searchMandal_Blocks('', 1, 10, '', '', true, 'm.Created_Date','string', '>', startDate, '').subscribe((result: any)=> {
...
(async(error: any) => {
      ...
      this.dataService.searchMandal_Blocks('', 1, 10, '', '', false, '','', '', '', '').subscribe((result: any)=> {
        this.mandal_blocksList = result.data;
        this.isLoading = false;
	},
(async(error: any) => {
  ...
  searchMandal_Blocks(searchText: any) {
    this.isLoading = true;
    this.dataService.searchMandal_Blocks(searchText, 1, 10, '', '', false, '','', '', '', '').subscribe((result: any)=> {
      this.mandal_blocksList = result.data;
       this.isLoading = false;
     },
(async(error: any) => {
...
setRecordId(selectedId: any) {
  ...
  deleteRecord(ev: any) {
    ...
    {
      this.isLoading = true;
      this.dataService.updateMandal_BlocksStatus(this.selectedId).subscribe(async(result: any)=> {
      this.isLoading = false;
	const toast = await this.toastController.create({
