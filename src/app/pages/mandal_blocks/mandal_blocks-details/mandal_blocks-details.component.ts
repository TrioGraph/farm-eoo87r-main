...
@Component({
  selector: 'app-mandal_blocks-details',
  templateUrl: './mandal_blocks-details.component.html',
  styleUrls: ['./mandal_blocks-details.component.css']
})
export class Mandal_BlocksDetailsComponent implements OnInit {

  public mandal_blocksDetails: any;
  id!: string;
  ionicForm!: FormGroup;
  formMode: any = 'View';
...
isActive: ['', [Validators.required]],
   ...
   this.dataService.getDistrictsLookup().subscribe((result: any) => { 
	 this.District_Id_data = result; 
}); 


   this.dataService.getMandal_BlocksById(this.id).subscribe((data: any)=> {
      this.mandal_blocksDetails = data;
  ...
  submitForm(): void {
   ...
   this.isLoading = true;
    this.dataService.updateMandal_Blocks(this.id, tempFormData).subscribe(async(data: any) => {
...
fileChange(event: any, propertyName: any) {
  ...
  uploadImage(propertyName: any) {
    ...
    formData.append('tableName', 'Mandal_Blocks');
