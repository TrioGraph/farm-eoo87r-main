...
import { VFarmers_LoginComponent } from './v-Farmers_Login.component';

describe('VFarmers_LoginComponent', () => {
  let component: VFarmers_LoginComponent;
  let fixture: ComponentFixture<VFarmers_LoginComponent>;
  ...
  beforeEach(() => {
    ...
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetch<<Farmers_Login>>ErrorMessage) => ({})
    });
    const appsServiceStub = () => ({
      validatePermissionWithRole: editTaskAcivity => ({}),
      get<<Farmers_Login>>: () => ({ pipe: () => ({ subscribe: f => f({}) }) }),
      deleteRecord: (string, guid) => ({ subscribe: f => f({}) })
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [VFarmers_LoginComponent],
      providers: [
        ...
        { provide: FarmService, useFactory: farmers_loginServiceStub }
      ]
    });
    fixture = TestBed.createComponent(VFarmers_LoginComponent);
  ...
  describe('fetchPermissions', () => {
    it('makes expected calls', () => {
      const farmers_loginServiceStub: Farmers_LoginService = fixture.debugElement.injector.get(
        FarmService
      );
      spyOn(farmers_loginServiceStub, 'validatePermissionWithRole').and.callThrough();
      component.fetchPermissions();
      expect(farmers_loginServiceStub.validatePermissionWithRole).toHaveBeenCalled();
    });
  });
  ...
  describe('fetchData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(farmers_loginServiceStub, 'getFarmers_Login').and.callThrough();
      component.fetchData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(farmers_loginServiceStub.getFarmers_Login).toHaveBeenCalled();
    });
  });
});
