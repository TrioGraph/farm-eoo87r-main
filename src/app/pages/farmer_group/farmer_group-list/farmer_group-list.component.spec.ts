...
import { VFarmer_GroupComponent } from './v-Farmer_Group.component';

describe('VFarmer_GroupComponent', () => {
  let component: VFarmer_GroupComponent;
  let fixture: ComponentFixture<VFarmer_GroupComponent>;
  ...
  beforeEach(() => {
    ...
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetch<<Farmer_Group>>ErrorMessage) => ({})
    });
    const appsServiceStub = () => ({
      validatePermissionWithRole: editTaskAcivity => ({}),
      get<<Farmer_Group>>: () => ({ pipe: () => ({ subscribe: f => f({}) }) }),
      deleteRecord: (string, guid) => ({ subscribe: f => f({}) })
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [VFarmer_GroupComponent],
      providers: [
        ...
        { provide: FarmService, useFactory: farmer_groupServiceStub }
      ]
    });
    fixture = TestBed.createComponent(VFarmer_GroupComponent);
  ...
  describe('fetchPermissions', () => {
    it('makes expected calls', () => {
      const farmer_groupServiceStub: Farmer_GroupService = fixture.debugElement.injector.get(
        FarmService
      );
      spyOn(farmer_groupServiceStub, 'validatePermissionWithRole').and.callThrough();
      component.fetchPermissions();
      expect(farmer_groupServiceStub.validatePermissionWithRole).toHaveBeenCalled();
    });
  });
  ...
  describe('fetchData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(farmer_groupServiceStub, 'getFarmer_Group').and.callThrough();
      component.fetchData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(farmer_groupServiceStub.getFarmer_Group).toHaveBeenCalled();
    });
  });
});
