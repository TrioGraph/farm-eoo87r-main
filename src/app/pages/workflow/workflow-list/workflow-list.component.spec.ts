...
import { VWorkflowComponent } from './v-Workflow.component';

describe('VWorkflowComponent', () => {
  let component: VWorkflowComponent;
  let fixture: ComponentFixture<VWorkflowComponent>;
  ...
  beforeEach(() => {
    ...
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetch<<Workflow>>ErrorMessage) => ({})
    });
    const appsServiceStub = () => ({
      validatePermissionWithRole: editTaskAcivity => ({}),
      get<<Workflow>>: () => ({ pipe: () => ({ subscribe: f => f({}) }) }),
      deleteRecord: (string, guid) => ({ subscribe: f => f({}) })
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [VWorkflowComponent],
      providers: [
        ...
        { provide: FarmService, useFactory: workflowServiceStub }
      ]
    });
    fixture = TestBed.createComponent(VWorkflowComponent);
  ...
  describe('fetchPermissions', () => {
    it('makes expected calls', () => {
      const workflowServiceStub: WorkflowService = fixture.debugElement.injector.get(
        FarmService
      );
      spyOn(workflowServiceStub, 'validatePermissionWithRole').and.callThrough();
      component.fetchPermissions();
      expect(workflowServiceStub.validatePermissionWithRole).toHaveBeenCalled();
    });
  });
  ...
  describe('fetchData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(workflowServiceStub, 'getWorkflow').and.callThrough();
      component.fetchData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(workflowServiceStub.getWorkflow).toHaveBeenCalled();
    });
  });
});
