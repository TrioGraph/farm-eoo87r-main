...
import { CWorkflowComponent } from './c-workflow.component';

describe('CWorkflowComponent', () => {
  let component: CWorkflowComponent;
  let fixture: ComponentFixture<CWorkflowComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchWorkflowErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getWorkflowById: value => ({ subscribe: f => f({}) }),
      updateWorkflow: (value, value1) => ({ subscribe: f => f({}) }),
      addWorkflow: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CWorkflowComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CWorkflowComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getWorkflowById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getWorkflowById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updateWorkflow').and.callThrough();
      spyOn(appsServiceStub, 'addWorkflow').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updateWorkflow).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addWorkflow).toHaveBeenCalled();
    });
  });
});
