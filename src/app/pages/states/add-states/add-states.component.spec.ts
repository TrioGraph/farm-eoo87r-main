...
import { CStatesComponent } from './c-states.component';

describe('CStatesComponent', () => {
  let component: CStatesComponent;
  let fixture: ComponentFixture<CStatesComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchStatesErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getStatesById: value => ({ subscribe: f => f({}) }),
      updateStates: (value, value1) => ({ subscribe: f => f({}) }),
      addStates: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CStatesComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CStatesComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getStatesById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getStatesById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updateStates').and.callThrough();
      spyOn(appsServiceStub, 'addStates').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updateStates).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addStates).toHaveBeenCalled();
    });
  });
});
