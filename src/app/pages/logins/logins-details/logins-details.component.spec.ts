...
import { CLoginsComponent } from './c-logins.component';

describe('CLoginsComponent', () => {
  let component: CLoginsComponent;
  let fixture: ComponentFixture<CLoginsComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchLoginsErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getLoginsById: value => ({ subscribe: f => f({}) }),
      updateLogins: (value, value1) => ({ subscribe: f => f({}) }),
      addLogins: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CLoginsComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CLoginsComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getLoginsById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getLoginsById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updateLogins').and.callThrough();
      spyOn(appsServiceStub, 'addLogins').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updateLogins).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addLogins).toHaveBeenCalled();
    });
  });
});
