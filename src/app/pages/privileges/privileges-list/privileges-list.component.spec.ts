...
import { VPrivilegesComponent } from './v-Privileges.component';

describe('VPrivilegesComponent', () => {
  let component: VPrivilegesComponent;
  let fixture: ComponentFixture<VPrivilegesComponent>;
  ...
  beforeEach(() => {
    ...
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetch<<Privileges>>ErrorMessage) => ({})
    });
    const appsServiceStub = () => ({
      validatePermissionWithRole: editTaskAcivity => ({}),
      get<<Privileges>>: () => ({ pipe: () => ({ subscribe: f => f({}) }) }),
      deleteRecord: (string, guid) => ({ subscribe: f => f({}) })
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [VPrivilegesComponent],
      providers: [
        ...
        { provide: FarmService, useFactory: privilegesServiceStub }
      ]
    });
    fixture = TestBed.createComponent(VPrivilegesComponent);
  ...
  describe('fetchPermissions', () => {
    it('makes expected calls', () => {
      const privilegesServiceStub: PrivilegesService = fixture.debugElement.injector.get(
        FarmService
      );
      spyOn(privilegesServiceStub, 'validatePermissionWithRole').and.callThrough();
      component.fetchPermissions();
      expect(privilegesServiceStub.validatePermissionWithRole).toHaveBeenCalled();
    });
  });
  ...
  describe('fetchData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(privilegesServiceStub, 'getPrivileges').and.callThrough();
      component.fetchData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(privilegesServiceStub.getPrivileges).toHaveBeenCalled();
    });
  });
});
