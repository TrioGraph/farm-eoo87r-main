...
export class PrivilegesListComponent  implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.allowToAdd = localStorage.getItem('AccessList')?.split(',').includes(Privileges.AddPrivileges.toString()) ? true : false; 
    this.allowToEdit = localStorage.getItem('AccessList')?.split(',').includes(Privileges.UpdatePrivileges.toString()) ? true : false;  
    this.allowToDelete = localStorage.getItem('AccessList')?.split(',').includes(Privileges.DeletePrivileges.toString()) ? true : false;  
  }

getData(startDate: any) {
    this.isLoading = true;
    if(startDate && startDate.Length > 0) {
        this.dataService.searchPrivileges('', 1, 10, '', '', true, 'm.Created_Date','string', '>', startDate, '').subscribe((result: any)=> {
...
(async(error: any) => {
      ...
      this.dataService.searchPrivileges('', 1, 10, '', '', false, '','', '', '', '').subscribe((result: any)=> {
        this.privilegesList = result.data;
        this.isLoading = false;
	},
(async(error: any) => {
  ...
  searchPrivileges(searchText: any) {
    this.isLoading = true;
    this.dataService.searchPrivileges(searchText, 1, 10, '', '', false, '','', '', '', '').subscribe((result: any)=> {
      this.privilegesList = result.data;
       this.isLoading = false;
     },
(async(error: any) => {
...
setRecordId(selectedId: any) {
  ...
  deleteRecord(ev: any) {
    ...
    {
      this.isLoading = true;
      this.dataService.updatePrivilegesStatus(this.selectedId).subscribe(async(result: any)=> {
      this.isLoading = false;
	const toast = await this.toastController.create({
