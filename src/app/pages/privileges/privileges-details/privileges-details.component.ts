...
@Component({
  selector: 'app-privileges-details',
  templateUrl: './privileges-details.component.html',
  styleUrls: ['./privileges-details.component.css']
})
export class PrivilegesDetailsComponent implements OnInit {

  public privilegesDetails: any;
  id!: string;
  ionicForm!: FormGroup;
  formMode: any = 'View';
...
isActive: ['', [Validators.required]],
   ...
   this.dataService.getPrivilegesById(this.id).subscribe((data: any)=> {
      this.privilegesDetails = data;
  ...
  submitForm(): void {
   ...
   this.isLoading = true;
    this.dataService.updatePrivileges(this.id, tempFormData).subscribe(async(data: any) => {
...
fileChange(event: any, propertyName: any) {
  ...
  uploadImage(propertyName: any) {
    ...
    formData.append('tableName', 'Privileges');
