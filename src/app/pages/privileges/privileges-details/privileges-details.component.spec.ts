...
import { CPrivilegesComponent } from './c-privileges.component';

describe('CPrivilegesComponent', () => {
  let component: CPrivilegesComponent;
  let fixture: ComponentFixture<CPrivilegesComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchPrivilegesErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getPrivilegesById: value => ({ subscribe: f => f({}) }),
      updatePrivileges: (value, value1) => ({ subscribe: f => f({}) }),
      addPrivileges: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CPrivilegesComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CPrivilegesComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getPrivilegesById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getPrivilegesById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updatePrivileges').and.callThrough();
      spyOn(appsServiceStub, 'addPrivileges').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updatePrivileges).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addPrivileges).toHaveBeenCalled();
    });
  });
});
