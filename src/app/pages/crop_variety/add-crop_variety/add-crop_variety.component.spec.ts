...
import { CCrop_VarietyComponent } from './c-crop_variety.component';

describe('CCrop_VarietyComponent', () => {
  let component: CCrop_VarietyComponent;
  let fixture: ComponentFixture<CCrop_VarietyComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchCrop_VarietyErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getCrop_VarietyById: value => ({ subscribe: f => f({}) }),
      updateCrop_Variety: (value, value1) => ({ subscribe: f => f({}) }),
      addCrop_Variety: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CCrop_VarietyComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CCrop_VarietyComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getCrop_VarietyById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getCrop_VarietyById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updateCrop_Variety').and.callThrough();
      spyOn(appsServiceStub, 'addCrop_Variety').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updateCrop_Variety).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addCrop_Variety).toHaveBeenCalled();
    });
  });
});
