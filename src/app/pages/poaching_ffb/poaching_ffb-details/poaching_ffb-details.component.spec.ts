...
import { CPoaching_FFBComponent } from './c-poaching_ffb.component';

describe('CPoaching_FFBComponent', () => {
  let component: CPoaching_FFBComponent;
  let fixture: ComponentFixture<CPoaching_FFBComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchPoaching_FFBErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getPoaching_FFBById: value => ({ subscribe: f => f({}) }),
      updatePoaching_FFB: (value, value1) => ({ subscribe: f => f({}) }),
      addPoaching_FFB: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CPoaching_FFBComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CPoaching_FFBComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getPoaching_FFBById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getPoaching_FFBById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updatePoaching_FFB').and.callThrough();
      spyOn(appsServiceStub, 'addPoaching_FFB').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updatePoaching_FFB).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addPoaching_FFB).toHaveBeenCalled();
    });
  });
});
