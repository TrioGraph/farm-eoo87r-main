...
import { VPoaching_FFBComponent } from './v-Poaching_FFB.component';

describe('VPoaching_FFBComponent', () => {
  let component: VPoaching_FFBComponent;
  let fixture: ComponentFixture<VPoaching_FFBComponent>;
  ...
  beforeEach(() => {
    ...
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetch<<Poaching_FFB>>ErrorMessage) => ({})
    });
    const appsServiceStub = () => ({
      validatePermissionWithRole: editTaskAcivity => ({}),
      get<<Poaching_FFB>>: () => ({ pipe: () => ({ subscribe: f => f({}) }) }),
      deleteRecord: (string, guid) => ({ subscribe: f => f({}) })
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [VPoaching_FFBComponent],
      providers: [
        ...
        { provide: FarmService, useFactory: poaching_ffbServiceStub }
      ]
    });
    fixture = TestBed.createComponent(VPoaching_FFBComponent);
  ...
  describe('fetchPermissions', () => {
    it('makes expected calls', () => {
      const poaching_ffbServiceStub: Poaching_FFBService = fixture.debugElement.injector.get(
        FarmService
      );
      spyOn(poaching_ffbServiceStub, 'validatePermissionWithRole').and.callThrough();
      component.fetchPermissions();
      expect(poaching_ffbServiceStub.validatePermissionWithRole).toHaveBeenCalled();
    });
  });
  ...
  describe('fetchData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(poaching_ffbServiceStub, 'getPoaching_FFB').and.callThrough();
      component.fetchData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(poaching_ffbServiceStub.getPoaching_FFB).toHaveBeenCalled();
    });
  });
});
