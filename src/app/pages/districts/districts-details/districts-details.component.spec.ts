...
import { CDistrictsComponent } from './c-districts.component';

describe('CDistrictsComponent', () => {
  let component: CDistrictsComponent;
  let fixture: ComponentFixture<CDistrictsComponent>;

  beforeEach(() => {
    const untypedFormBuilderStub = () => ({ group: object => ({}) });
    const nzDrawerRefStub = () => ({ close: data => ({}) });
    const nzNotificationServiceStub = () => ({
      create: (string, string1, fetchDistrictsErrorMessage) => ({})
    });
    const FarmServiceStub = () => ({
      getDistrictsById: value => ({ subscribe: f => f({}) }),
      updateDistricts: (value, value1) => ({ subscribe: f => f({}) }),
      addDistricts: value => ({ subscribe: f => f({}) }),
      resetForm: (e, appsForm) => ({})
    });
    TestBed.configureTestingModule({
      schemas: [NO_ERRORS_SCHEMA],
      declarations: [CDistrictsComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(CDistrictsComponent);
  ...
  describe('setFormData', () => {
    it('makes expected calls', () => {
      ...
      spyOn(appsServiceStub, 'getDistrictsById').and.callThrough();
      component.setFormData();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.getDistrictsById).toHaveBeenCalled();
    });
  });
  ...
  describe('submitForm', () => {
    it('makes expected calls', () => {
      ...
      spyOn(nzNotificationServiceStub, 'create').and.callThrough();
      spyOn(appsServiceStub, 'updateDistricts').and.callThrough();
      spyOn(appsServiceStub, 'addDistricts').and.callThrough();
      component.submitForm();
      expect(nzDrawerRefStub.close).toHaveBeenCalled();
      expect(nzNotificationServiceStub.create).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.updateDistricts).toHaveBeenCalled();
      expect(<<ProjecteInstanceName>>ServiceStub.addDistricts).toHaveBeenCalled();
    });
  });
});
