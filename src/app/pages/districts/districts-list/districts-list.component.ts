...
export class DistrictsListComponent  implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.allowToAdd = localStorage.getItem('AccessList')?.split(',').includes(Privileges.AddDistricts.toString()) ? true : false; 
    this.allowToEdit = localStorage.getItem('AccessList')?.split(',').includes(Privileges.UpdateDistricts.toString()) ? true : false;  
    this.allowToDelete = localStorage.getItem('AccessList')?.split(',').includes(Privileges.DeleteDistricts.toString()) ? true : false;  
  }

getData(startDate: any) {
    this.isLoading = true;
    if(startDate && startDate.Length > 0) {
        this.dataService.searchDistricts('', 1, 10, '', '', true, 'm.Created_Date','string', '>', startDate, '').subscribe((result: any)=> {
...
(async(error: any) => {
      ...
      this.dataService.searchDistricts('', 1, 10, '', '', false, '','', '', '', '').subscribe((result: any)=> {
        this.districtsList = result.data;
        this.isLoading = false;
	},
(async(error: any) => {
  ...
  searchDistricts(searchText: any) {
    this.isLoading = true;
    this.dataService.searchDistricts(searchText, 1, 10, '', '', false, '','', '', '', '').subscribe((result: any)=> {
      this.districtsList = result.data;
       this.isLoading = false;
     },
(async(error: any) => {
...
setRecordId(selectedId: any) {
  ...
  deleteRecord(ev: any) {
    ...
    {
      this.isLoading = true;
      this.dataService.updateDistrictsStatus(this.selectedId).subscribe(async(result: any)=> {
      this.isLoading = false;
	const toast = await this.toastController.create({
